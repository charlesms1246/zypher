...
pub fn compute_position_hash(
    ...
    data.extend_from_slice(&minted_zypher.to_le_bytes());
...
pub fn simulate_mpc_share(secret: &[u8], t: usize, n: usize) -> Result<Vec<Vec<u8>>> {
    ...
    require!(n >= t, crate::errors::ZypherError::InvalidMPCParams);
    require!(t > 0 && n > 0, crate::errors::ZypherError::InvalidMPCParams);
...
pub fn simulate_mpc_reconstruct(
    ...
    require!(shares.len() >= t, crate::errors::ZypherError::TooFewShares);
    ...
    for share in shares.iter().take(t) {
        require!(share.len() == share_len, crate::errors::ZypherError::DeserializationError);
        for (i, &byte) in share.iter().enumerate() {
