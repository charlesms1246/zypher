...
import { ZypherProtocol } from "../target/types/zypher";
...
describe("zypher", () => {
  ...
  const program = anchor.workspace.ZypherProtocol as Program<ZypherProtocol>;
  
  let admin: Keypair;
  let user: Keypair;
  let collateralMint: PublicKey;
  let zypherMint: PublicKey;
  let userCollateralToken: PublicKey;
  let userZypherToken: PublicKey;
  ...
  before(async () => {
    ...
    userZypherToken = await createAccount(
      provider.connection,
      user,
      zypherMint,
      user.publicKey
  ...
  describe("Mint AEGIS", () => {
    it("Mints AEGIS with sufficient collateral", async () => {
      ...
      try {
        await program.methods
          .mintZypher(collateralIndex, depositAmount, mintAmount)
          .accounts({
        ...
        assert.ok(position.mintedZypher.eq(mintAmount));

        // Check token balances
        const userZypherAccount = await getAccount(
          provider.connection,
          userZypherToken
        );
        assert.equal(userZypherAccount.amount.toString(), mintAmount.toString());
      } catch (err) {
  ...
  describe("Burn AEGIS", () => {
    it("Burns AEGIS and updates position", async () => {
      ...
      const mintedBefore = positionBefore.mintedZypher;

      try {
        await program.methods
          .burnZypher(burnAmount)
          .accounts({
            position: positionPda,
            config: configPda,
            user: user.publicKey,
        ...
        assert.ok(
          positionAfter.mintedZypher.eq(mintedBefore.sub(burnAmount))
        );
      } catch (err) {
  ...
  describe("Integration Tests", () => {
    it("Complete flow: mint -> hedge -> burn", async () => {
      ...
      const integrationZypher = await createAccount(
        provider.connection,
        integrationUser,
        zypherMint,
        integrationUser.publicKey
      ...
      await program.methods
        .mintZypher(0, depositAmount, mintAmount)
        .accounts({
          ...
          vaultTokenAccount: vaultTokenAccount,
          zypherMint: zypherMint,
          userZypherToken: integrationZypher,
          oracleAccount: oracleAccount,
          tokenProgram: TOKEN_PROGRAM_ID,
          systemProgram: SystemProgram.programId,
        })
      ...
      position = await program.account.userPosition.fetch(integrationPosition);
      assert.ok(position.mintedZypher.eq(mintAmount.sub(burnAmount)));
